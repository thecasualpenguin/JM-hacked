 ...
 ************************************************************************
 * \brief
 *    Felix Injects, some script for testing
 *    and pulling out MV and texture
 ************************************************************************
 */
char* function_that_makes_pretty_output(objectBuffer_t *buf) {
    char* res = (char*) malloc(100*sizeof(char));
    
    sprintf(res,
            "mode: %d, x: %d, y: %d, z: %d",
            buf->regionMode, buf->mv[0], buf->mv[1], buf->mv[2]);
...
void ercWriteMBMODEandMV(Macroblock *currMB)
{
  ...
  StorablePicture *dec_picture = p_Vid->dec_picture;
    ...
    snprintf(buf, size_of_buf, "x: %d y: %d | pix_c_x: %d pix_c_y: %d | block_x: %d block_y: %d | mb_x: %d mb_y: %d \n",
           ...
           (*currMB).pix_c_x, (*currMB).pix_c_y,
           (*currMB).block_x, (*currMB).block_y,
             currMB->mb.x, currMB->mb.y);
    ...
    for (i=0; i<4; ++i)
    {
      ...
      else
      {
        ...
        char *mode_name = (pRegion->regionMode == REGMODE_INTRA ? "DEFAULT_INTRA"  :
                           pRegion->regionMode == REGMODE_INTRA_8x8 ? "INTRA_8x8"  :
                           pRegion->regionMode == REGMODE_INTRA_8x8 ? "INTRA_8x8"  :
                           pRegion->regionMode == REGMODE_INTER_COPY_8x8 ? "COPY_8x8"  :
                           pRegion->regionMode == REGMODE_INTER_PRED ? "INTER_PRED"  : "INTER_PRED_8x8");
